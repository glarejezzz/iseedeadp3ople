...
private:
    ...
    void broadcast_message(const std::string& message, const std::string& sender_nickname, 
                          const std::string& sender_ip_port, ClientInfo* sender_info = nullptr) {
        ...
        // Извлекаем текст сообщения для лога (без [nickname] )
        std::string log_message = message;
        size_t bracket_pos = message.find(']');
        if (bracket_pos != std::string::npos && bracket_pos + 2 < message.length()) {
            log_message = message.substr(bracket_pos + 2);
        }
        // Удаляем перевод строки
        while (!log_message.empty() && (log_message.back() == '\n' || log_message.back() == '\r')) {
            log_message.pop_back();
        }
        
        write_log(sender_ip_port, sender_nickname, log_message);
    }
...
public:
    ...
    void handle_client(std::unique_ptr<Socket> client_socket, const std::string& ip, int port) {
        ...
        // Получаем указатель на добавленного клиента
